...
export function HeroSection({ searchTerm, searchMessage, onSearch, onSearchTermChange }: HeroSectionProps) {
	return (
		<section className="relative isolate min-h-[calc(100svh-76px)] overflow-hidden bg-[linear-gradient(115deg,var(--citi-navy)_0%,var(--citi-blue-dark)_58%,var(--citi-blue)_100%)] text-white md:min-h-[calc(100vh-76px)]">
			...
			<div className="absolute bottom-0 right-[-2%] -z-10 h-[48%] w-full overflow-hidden md:top-0 md:h-full md:w-[38%]">
				<div className="absolute right-[18%] top-[18%] h-[38%] w-[54%] rounded-full bg-[var(--citi-yellow)]/20 blur-3xl" />
				<Image src="/images/sol 2.png" alt="Sol com faixas azuis e amarelas, símbolo de energia e inovação" width={1280} height={1280} priority className="absolute left-[10%] top-[8%] h-[78%] w-[78%] max-w-none object-contain md:left-[15%] md:top-[10%] md:h-[72%] md:w-[72%]" />
			</div>
			<div className="relative mx-auto flex min-h-[calc(100svh-76px)] max-w-7xl items-center justify-center px-5 pb-48 pt-14 sm:px-8 sm:pb-44 sm:pt-16 md:min-h-[calc(100vh-76px)] md:px-10 md:py-12">
				<div className="relative z-10 w-full max-w-[700px] text-left md:w-[62%] md:pr-6">
					...
					<h1 className="max-w-full font-[family-name:var(--font-joan)] text-[clamp(2.75rem,9vw,7rem)] leading-none">
						Portal <span className="text-[var(--citi-yellow)]">CiTI-DE</span>
					</h1>
					<p className="mt-6 max-w-[40rem] text-2xl leading-tight text-white sm:text-3xl">O que você precisa? Encontre serviços, programas e informações para impulsionar o desenvolvimento de Patos.</p>
					<p className="mt-4 max-w-[40rem] text-base leading-7 text-white/80 sm:text-lg">Acesse cursos, empregos, programas de inovação e incentivos para empresas.</p>
					...
					<form onSubmit={onSearch} className="mt-7 max-w-[720px]">
					...
					<div className="mt-7 flex w-full max-w-[720px] flex-wrap gap-2.5 sm:gap-3">

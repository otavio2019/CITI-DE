...
export function AudienceSection() {
	...
	return (
		<section id="publicos" className="bg-transparent px-5 py-20 text-white">
			<div className="mx-auto max-w-6xl"><div className="max-w-2xl"><p className="text-sm font-bold uppercase tracking-[0.18em] text-[var(--citi-yellow-soft)]">Escolha seu perfil</p><h2 className="mt-3 text-4xl leading-tight sm:text-5xl">O que você procura?</h2><p className="mt-4 text-lg leading-8 text-white/75">Selecione um caminho para encontrar rapidamente os serviços e oportunidades mais relevantes para você.</p></div>
				<div className="mt-10 grid gap-4 md:grid-cols-3">{(Object.entries(audiencePaths) as [keyof typeof audiencePaths, (typeof audiencePaths)[keyof typeof audiencePaths]][]).map(([key, path]) => <button key={key} type="button" onClick={() => setAudience(key)} aria-pressed={audience === key} className={`group rounded-2xl border p-6 text-left transition ${audience === key ? "border-[var(--citi-yellow)] bg-[var(--citi-blue-dark)] text-white shadow-[0_16px_40px_rgba(3,40,66,0.32)]" : "border-white/20 bg-white/5 text-white hover:border-[var(--citi-yellow-soft)] hover:bg-white/10"}`}><div className="flex items-center justify-between gap-4"><div className={`flex h-12 w-12 items-center justify-center rounded-xl ${audience === key ? "bg-[var(--citi-yellow)] text-[var(--citi-navy)]" : "bg-[var(--citi-blue)] text-white"}`}>{key === "empresa" ? <FiBriefcase aria-hidden="true" size={22} /> : <FiUsers aria-hidden="true" size={22} />}</div><FiArrowUpRight aria-hidden="true" className={`text-2xl transition-transform group-hover:translate-x-1 group-hover:-translate-y-1 ${audience === key ? "text-[var(--citi-yellow)]" : "text-[var(--citi-yellow-soft)]"}`} /></div><h3 className="mt-6 text-2xl">{path.label}</h3><p className="mt-2 leading-7 text-white/70">{path.intro}</p></button>)}</div>
